...
verify_ck(){
    ...
    check_ck(){
        ...
        local notify=$2
        local j=${pin[i]}
        local jd_cookie emoji timestamp_ms
        ...
        if [[ ${ck_status[$j]} = 0 ]] && [[ ${status_json[$j]} = 1 ]]; then
